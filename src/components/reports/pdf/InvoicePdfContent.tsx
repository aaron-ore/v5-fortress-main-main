...
const InvoicePdfContent: React.FC<InvoicePdfContentProps> = ({
  ...
  const safeCustomerName = escapeHtml(customerName ?? "N/A");
  const safeCustomerEmail = escapeHtml(customerEmail ?? "N/A");
  const safeCustomerAddressLines = (customerAddress ?? "N/A").split('\n').map(escapeHtml);
    ...
    <div className="bg-white text-gray-900 font-sans text-sm p-[20mm]">
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-semibold">INVOICE #: {safeInvoiceNumber}</p>
          {invoiceQrCodeSvg && (
      ...
      <div className="grid grid-cols-2 gap-8 mb-8">
        ...
        <div>
          ...
          <div className="bg-gray-50 p-3 border border-gray-200 rounded">
            <p className="font-semibold">{safeCustomerName}</p>
            <p>{safeCustomerEmail}</p>
            <p>{safeCustomerAddressLines[0]}</p>
            <p>{safeCustomerAddressLines[1]}</p>
          </div>
        </div>
      </div>
