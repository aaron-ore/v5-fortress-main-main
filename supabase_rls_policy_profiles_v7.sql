...
USING (
  ...
  (
    ...
    AND
    (
      (public.get_user_organization_id() = old.organization_id) -- Admin can update profiles already in their org
      OR
      (old.organization_id IS NULL AND public.get_user_organization_id() IS NOT NULL) -- Admin can update profiles with NULL org_id (new users)
    )
  )
)
WITH CHECK (
