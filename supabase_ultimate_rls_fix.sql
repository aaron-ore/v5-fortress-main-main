-- Step 0: Temporarily Disable RLS on the profiles table to allow policy manipulation
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;

-- Step 1: Explicitly drop the problematic policy by its exact reported name
...
-- Step 2: Drop all other policies by their generic names (as a comprehensive cleanup)
-- This ensures no other old policies are lingering.
...
-- Step 3: Drop the function using CASCADE to automatically remove any remaining dependent objects
-- This should now work as the explicitly named problematic policy is gone.
DROP FUNCTION IF EXISTS public.get_user_organization_id() CASCADE;

-- Step 4: Create or replace the function to get the current user's organization_id
...
-- Step 6: Re-apply the REVISED RLS policies with explicit NULL handling
